...
Deno.serve(async (req) => {
  ...
  try {
    const loaded = await loadAppointmentEmailData(supabase, agendamento.id)
    if (loaded) {
      await sendEmail(loaded.negocio, {
        to: loaded.templateData.profissional_email,
        template: 'novo_agendamento_profissional',
        data: loaded.templateData,
      })
      await sendEmail(loaded.negocio, {
        to: loaded.templateData.cliente_email,
        template: 'confirmacao_cliente',
        data: loaded.templateData,
      })
      await supabase
        .from('agendamentos')
        .update({ confirmacao_email_enviada_em: new Date().toISOString() })
